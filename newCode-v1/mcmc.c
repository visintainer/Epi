...
int main(){
  ...
  int REP = 100000;
  int rep = 0;
  double alphastart = 0;//-2.05906;//-2.6;//0.002; ///0.001563487;///0.002; /// 0.001563487;
  double betastart = -0.4;//-0.2218487;//-0.58787;///0.3; /// 0.2873568;
  ...
  fpd=fopen("ORIG-date-fake.dat","r");
  ...
  //chiamo sir
  //sprintf(Command,"./sir %s %f %f",rseed, newalpha, newbeta);
  //fprintf(stderr,"%s\n",Command);
  //merda = system(Command); 
  
  for(i=0;i<N;i++)  
    newday[i]=0;
  //nIsim = sir(powf(10.,Alpha[rep]), powf(10.,Beta[rep]), rseed, newidx, newday);
  nIsim = sir(0, powf(10.,Beta[rep]), rseed, newidx, newday);
  //  acceptednIsim=nIsim;
  for(i=0;i<N;i++)  
    acceptedday[i]=0;
  for(i=0;i<nIsim;i++)  
  ...
  while (newlkh == 0){ //Prima Likelihood fin che e' == a 0
    ...
    nIsim = sir(0, powf(10.,newbeta), rseed, newidx, newday);
  ...
  for (rep=1; rep<REP; rep++){ //REPLICHE
    ...
    if((accept >= 1) || (lkh[rep-1] == 0) || (gsl_ran_flat(R_GLOBAL,0,1)<accept)){ // Se nuova lkh > della vecchia o la vecchia e' 0 o pesco male

      //      fprintf(stdout,"accetto\n");
      //      acceptednIsim=nIsim;
      for(i=0;i<N;i++)  
	acceptedday[i]=0;
      for(i=0;i<nIsim;i++)  
	...
	//	fprintf(stderr,"accetto e ci riprovo\t");
	// Ricalcolo sir
	//nIsim = sir(powf(10.,newalpha), powf(10.,newbeta), rseed, newidx, newday);
	for(i=0;i<N;i++)  
	  newday[i]=0;
	...
	fprintf(foutsim, "%e\t",acceptedday[i]);
      ...
      newlkh = 0;
      while(newlkh ==0){ // fin che non parte l'epidemia
	//	fprintf(stderr,"rifiuto e ci riprovo\t");
	// Ricalcolo sir
	//      nIsim = sir(powf(10.,Alpha[rep-1]), powf(10.,Beta[rep-1]), rseed, newidx, newday);
	for(i=0;i<N;i++)  
	  newday[i]=0;
	nIsim = sir(0, powf(10.,Beta[rep-1]), rseed, newidx, newday);
	// ricalcolo LKH
	newlkh = likelihood(nIsim, day_data, newday); //Likelihood
	//	fprintf(stderr,"lkh %Le \n",newlkh);
      }
    ...
    /* if(accept >=1){ */
    /*   //    fprintf(stderr,"POWER rep:%d L:%Le logL:%e A:%e B:%e newA:%e newB:%e\n",rep,lkh[rep],log(lkh[rep]),powf(10.,Alpha[rep]),powf(10.,Beta[rep]),powf(10.,newalpha),powf(10.,newbeta)); */
    /* //  fprintf(stderr,"rep:%d L:%Le logL:%e A:%e B:%e newA:%e newB:%e\n",rep,lkh[rep],log(lkh[rep]),Alpha[rep],Beta[rep],newalpha,newbeta); */
    /* } */
  }

...
int main(){
  double threshold=1e-120;
  MAXTIME = 100;
  int REP = 10000;
  double alphastart = 0.;//-2.;//-2.8;//-2.05906;//-2.6;//0.002; ///0.001563487;///0.002; /// 0.001563487;
  double betastart = -0.06;//-0.5228787;//-0.2218487;//-0.58787;///0.3; /// 0.2873568;
  double psymstart = 1;
  ...
  double deltapsym = 0.;
  double newalpha, newbeta, newpsym; 
  double penalita = 0.1;
  long double accept;
  long double newlkh;
  long double tmplkh;
  ...
  j=0;
  for (i=0;i<nIsim;i++){
    if (gsl_ran_flat(R_GLOBAL,0,1) < Psym[rep]){ // SE HO I SINTOMI
      newday_withsymp[j] = newday[i];
      j++;
    }
  }
  nIsim = j-1;
  ...
  for(i=0;i<N;i++)  
    acceptedday[i]=0;
  for(i=0;i<nIsim;i++)  
    acceptedday[i] = newday_withsymp[i];
  for (i=0; i<nIsim; i++)
    fprintf(stderr,"newday %e\n",newday_withsymp[i]);
  
  newlkh = likelihood(nIsim, day_data, newday_withsymp); //Prima Likelihood
  ...
  while (newlkh == 0){ //Prima Likelihood fin che e' == a 0
    ...
    newpsym = Psym[rep]+(deltabeta*gsl_ran_gaussian(R_GLOBAL,1.));
    for(i=0;i<N;i++){
      newday[i]=0;
      newday_withsymp[i] = 0;
    }
    ...
    j=0;
    for (i=0;i<nIsim;i++){
      if (gsl_ran_flat(R_GLOBAL,0,1) < newpsym){ // SE HO I SINTOMI
	newday_withsymp[j] = newday[i];
	j++;
      }
    }
    nIsim = j-1;
    newlkh = likelihood(nIsim, day_data, newday_withsymp); 
    count ++;
  }
  Alpha[rep] = newalpha;
  Beta[rep] = newbeta;
  ...
  for (rep=1; rep<REP; rep++){ //REPLICHE
    ...
    while(tmplkh ==0){ // fin che non parte l'epidemia
      ...
      newpsym = Psym[rep-1]+(deltapsym*gsl_ran_gaussian(R_GLOBAL,1.));
      // Chiamo sir
      for(i=0;i<N;i++){ 
	newday[i]=0;
	newday_withsymp[i]=0;
	...
	if (gsl_ran_flat(R_GLOBAL,0,1) < newpsym){ // SE HO I SINTOMI
      ...
      nIsim = j-1;
      tmplkh = likelihood(nIsim, day_data, newday_withsymp); //Likelihood
    }
	...
	nIsim = j-1;
	// Ricalcolo LKH
	newlkh = likelihood(nIsim, day_data, newday_withsymp); //Likelihood
      }
	...
	if(tmplkh>threshold){
      ...
      fprintf(fout, "%d\t%Le\t%e\t%e\t%e\t%e\n",rep,acceptedlkh,log(acceptedlkh),acceptedAlpha,acceptedBeta,acceptedPsym);
      fclose(fout);
